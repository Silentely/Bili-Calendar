...
- 🌍 **多语言支持**：支持简体中文、English、繁體中文、日本語界面循环切换
- 📱 **PWA支持**：可安装为独立应用，支持离线访问与上次预览回退
- 🎨 **增强UI/UX**：精美动画效果、加载状态、错误引导、订阅向导
- ⌨️ **键盘快捷键**：支持快捷操作，提升使用效率
- 📱 **移动优化**：完美适配移动设备，触摸优化
- 🔔 **推送提醒**：预览面板可选择提前 5/10/15 分钟本地提醒；WebPush 需自托管配置 VAPID（Netlify 无状态无法持久化）
- 📊 **监控指标**：`/metrics` JSON 与 `/metrics/prometheus`；含 p95/p99、路由级与聚合源统计
- 🔀 **外部日历聚合**：通过 `/aggregate/:uid.ics?sources=` 合并最多 5 个外部 ICS 链接
...
| `TRUST_PROXY`              | 主服务未启用；Netlify 默认 `1` | 可选，配置 Express `trust proxy`（true/false/跳数/IP）。Netlify 还会优先读 `x-nf-client-connection-ip` |
...
4. **推送提醒**：执行 `node scripts/generate-vapid.js` 生成 VAPID 公私钥并设置上述环境变量，然后前端点击"启用推送"完成订阅；未配置时按钮会提示失败
...
7. **反向代理信任**：主服务默认不信任 `X-Forwarded-For`；Netlify Functions 默认信任 1 层代理，并优先使用平台头 `x-nf-client-connection-ip`。自建代理/CDN 请显式设置 `TRUST_PROXY`（如 `true`、`1` 或具体子网）
...
- `X-RateLimit-Backend`: 限流后端类型（当前为 `memory`；Serverless 多实例下为尽力而为）
- 聚合响应额外包含：`X-Aggregate-Sources-Requested` / `Fetched` / `Failed`
...
- 📚 追番列表分页拉全（最多 20 页 × 30 条），避免重度用户被截断
- 📅 ICS：连载 `RRULE COUNT=52`、`DTEND`、默认 15 分钟 `VALARM`；响应增加 CDN 缓存头
- 🧩 抽出 `server/lib` 共享 Handler/中间件，Express 与 Netlify 行为对齐
- 🏷️ `utils/*.cjs` 标注弃用，运行时以 `utils-es` 为唯一真相源
- 🌍 i18n 扩展繁體中文 / 日本語（关键文案 + 回退简体）
- 🧭 订阅成功区增加三步向导；WebPush 明确自托管限制
- 📴 PWA 缓存版本升级；预览失败可回退本地上次预览
- 📊 metrics 增加外部 ICS 聚合源计数；聚合响应输出源状态头
